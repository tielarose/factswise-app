def create_add_sub_problem_set_questions(list_of_part_part_wholes):
    ...
    Output should be: [(string_question, string_answer), (string_question, string_answer)]
    ...
    for (part1, part2, whole) in list_of_part_part_wholes:
        if part1 != part2:
            problem_set_questions.append((f"{part1} + {part2}", f"{whole}"))
            problem_set_questions.append((f"{part2} + {part1}", f"{whole}"))
            problem_set_questions.append((f"{whole} - {part1}", f"{part2}"))
            problem_set_questions.append((f"{whole} - {part2}", f"{part1}"))
        else:
            problem_set_questions.append((f"{part1} + {part2}", f"{whole}"))
            problem_set_questions.append((f"{whole} - {part1}", f"{part2}"))
...
def create_mult_div_problem_set_questions(list_of_factor_factor_products):
    ...
    for (factor1, factor2, product) in list_of_factor_factor_products:
        if factor1 != factor2:
            problem_set_questions.append((f"{factor1} * {factor2}", f"{product}"))
            problem_set_questions.append((f"{factor2} * {factor1}", f"{product}"))
            problem_set_questions.append((f"{product} / {factor1}", f"{factor2}"))
            problem_set_questions.append((f"{product} / {factor2}", f"{factor1}"))
        else:
            problem_set_questions.append((f"{factor1} * {factor2}", f"{product}"))
            problem_set_questions.append((f"{product} / {factor1}", f"{factor2}"))
    ...
    },
{
    ...
    "list_of_part_part_wholes": [(1,5,6),(2,5,7),(3,5,8),(4,5,9),(5,5,10)]
    },
{
    "problem_set_type_id": 1,
    "problem_set_type_desciption": "Addition Subtraction",
    ...
    "list_of_part_part_wholes": [(0,10,10), (1,9,10), (2,8,10), (3,7,10),(4,6,10)]
    },
{
    ...
    "list_of_factor_factor_products": [(10,2,20),(10,3,30),(10,4,40),(10,5,50),(10,6,60),(10,7,70),(10,8,80),(10,9,90),(10,10,100)]
    },
{
    "problem_set_type_id": 2,
    "problem_set_type_desciption": "Multiplication Division",
    ...
    "list_of_factor_factor_products": [(2,5,10),(3,5,15),(4,5,20),(5,5,25),(6,5,30),(7,5,35),(8,5,40),(9,5,45)]
    },
{
    "problem_set_type_id": 2,
    "problem_set_type_desciption": "Multiplication Division",
    ...
    "list_of_factor_factor_products": [(2,2,4),(2,3,6),(2,4,8),(2,6,12),(2,7,14),(2,8,16),(2,9,18)]
    }
]